...
with CoreNLPClient(annotators=['entitylink'], timeout=300000, memory='16G', threads=32) as client:
    directory_in_str = "/data1/chwongcc/stanza/demo/corpus_full_ph"
    ...
    for file in os.listdir(directory):
        ...
        with open(f"{directory_in_str}/{filename}") as r:
            ...
            # if (len(vocab) != len(filename_wo_txt)-1):
            #     continue
            corpus=""
            lines = r.readlines()
            for i, line in enumerate(lines):
                # print(line)
                if (i == 0 or re.match("^h[0-9]", line)):
                    continue
                corpus += line.replace('\r', '') 
        ...
        # print('Found %s triples in the corpus.' % len(triples_corpus))
            ...
            for sentence in ann.sentence:
                for token in sentence.token:
                    triple_dict = {}
                    triple_dict['token'] =token.word
                    triple_dict['wiki_entity'] = token.wikipediaEntity
        ...
        # if flag_vocab:
        header = ['token','wiki_entity']
    
        with open(f'/data1/chwongcc/stanza/demo/wikientity_csv_full_ph/{filename[:-4]}.csv', 'w') as csvfile:
